...
module.exports = async (config, options, targetOptions) => {
  ...
  if (config.mode === 'development') {
    config.plugins.push(
      new WebpackNotifierPlugin({
        title: 'Jhipster Oauth 2 Sample Application',
          ...
          proxy: {
            ...
            proxyReq: [
              function (proxyReq) {
                ...
                // Tomcat's RemoteIpValve drops the port from X-Forwarded-Host, it must be forwarded separately
  ...
  config.plugins.push(
    new webpack.DefinePlugin({
      ...
      'process.env.SERVER_API_URL': JSON.stringify(environment.SERVER_API_URL),
    }),
